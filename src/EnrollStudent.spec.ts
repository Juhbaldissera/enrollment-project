...
describe('Enroll student', () => {
    ...
    it('Should generate the invoices based on the number of installments, rounding each amount and applying the rest in the last invoice', () => {
        const enrollStudent = new EnrollStudent(enrollmentRepository, classesRepository);
        const lastEnrollment = enrollStudent.execute(enrollmentRequestSample);
        expect(lastEnrollment.installments[0]).toEqual(1416.66);
        expect(lastEnrollment.installments[1]).toEqual(1416.66);
        expect(lastEnrollment.installments[2]).toEqual(1416.66);
        expect(lastEnrollment.installments[3]).toEqual(1416.66);
        expect(lastEnrollment.installments[4]).toEqual(1416.66);
        expect(lastEnrollment.installments[5]).toEqual(1416.66);
        expect(lastEnrollment.installments[6]).toEqual(1416.66);
        expect(lastEnrollment.installments[7]).toEqual(1416.66);
        expect(lastEnrollment.installments[8]).toEqual(1416.66);
        expect(lastEnrollment.installments[9]).toEqual(1416.66);
        expect(lastEnrollment.installments[10]).toEqual(1416.66);
        expect(lastEnrollment.installments[11]).toEqual(1416.73);
    });
});
